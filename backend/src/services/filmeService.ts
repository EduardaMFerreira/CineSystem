...
export const filmeService = {
  ...
  create: async (data: { titulo: string; genero: string; duracao: number; bannerUrl: string }) => {
    return await prisma.filme.create({ data });
  },

  update: async (id: number, data: { titulo?: string; genero?: string; duracao?: number; bannerUrl: string }) => {
